mod decrypt;
mod encrypt;
mod key;
pub mod mode;
mod state;

#[cfg(test)]
mod test;

use std::ops::Div;

use self::{mode::Mode, state::State};
use crate::{xor::fixed_xor, BoxedError};

pub const SUB_BYTES_TABLE: [u8; 256] = [
    ,
];

pub struct AES {
    pub iv: Option<State>,
    pub mode: Mode,
    //
    pub key: [u8; 16],
    //
}

impl AES {
    pub fn new(iv: Option<State>, key: [u8; 16], mode: Mode) -> AES {
        AES { iv, key, mode }
    }

    fn add_round_key(
        &self,
        prev_state: State,
        round_key: [u8; 16],
    ) -> Result<State, BoxedError> {
        let next_state =
            fixed_xor(prev_state.state.to_vec(), round_key.to_vec())?;

        Ok(State::from_vec(next_state)?)
    }

    pub fn encrypt(&self, mut data: Vec<u8>) -> Result<Vec<u8>, BoxedError> {
        // 1. get extended key
        let ext_key: [[u8; 16]; 11] = self.key_expansion()?;

        // 2. divide data into blocks
        let mut res_state: Vec<State> = Vec::<State>::new();

        let mut blocks: Vec<State> = Vec::<State>::new();

        let mut tmp_block = Vec::<u8>::new();

        while data.is_empty() == false {
            if tmp_block.len() == 16 {
                let mut tmp_state = [0u8; 16];

                for idx in 0..16 {
                    tmp_state[idx] = tmp_block[idx];
                }

                blocks.push(State { state: tmp_state });

                tmp_block.clear();
            } else {
                tmp_block.push(data.remove(0));
            }
        }

        while tmp_block.is_empty() == false && tmp_block.len() < 16 {
            tmp_block.push(0u8);
        }

        if tmp_block.is_empty() == false {
            let mut tmp_state = [0u8; 16];

            for idx in 0..16 {
                tmp_state[idx] = tmp_block[idx];
            }

            blocks.push(State { state: tmp_state });
        }

        // 3. round
        for block in blocks {
            let mut state = block;
            println!("\n[debug] init state: {:02x?}", state.state);
            // round 0
            state = self.add_round_key(state, ext_key[0])?;
            println!("[debug] after round 0 ark: {:02x?}", state.state);

            // round 1 ~ 9
            for key_idx in 1..10 {
                // byte sub
                println!("\n[debug] round {}", key_idx);

                state = self.sub_bytes(state);
                println!("[debug] after sub: {:02x?}", state.state);

                // shift rows
                state = self.shift_rows(state);
                println!("[debug] after sft: {:02x?}", state.state);

                // mix columns
                state = self.mix_columns(state)?;
                println!("[debug] after mix: {:02x?}", state.state);

                // add round key
                state = self.add_round_key(state, ext_key[key_idx])?;
                println!("[debug] round key: {:02x?}", ext_key[key_idx]);
                println!("[debug] after ark: {:02x?}", state.state);
            }

            // round 10
            // byte sub
            state = self.sub_bytes(state);

            // shift rows
            state = self.shift_rows(state);

            // add round key
            state = self.add_round_key(state, ext_key[10])?;

            println!("\n[debug] round 10");
            println!("[debug] after ark: {:02x?}", state.state);

            res_state.push(state);
        }

        let mut res: Vec<u8> = vec![];

        for state in res_state {
            res.append(&mut state.state.to_vec());
        }

        Ok(res)
    }

    // pub fn decrypt(&self, key: [u8; 16]) -> Vec<u8> {
    //     self.data.clone()
    // }

    fn xtime(&self, x: u8, byte: u8) -> Result<u8, BoxedError> {
        let res = match x {
            1 => self.xtime_1(byte)?,
            2 => self.xtime_2(byte)?,
            3 => self.xtime_3(byte)?,
            _ => {
                return Err(format!("Not implemented yet").into());
            }
        };

        Ok(res)
    }

    #[inline]
    fn xtime_1(&self, byte: u8) -> Result<u8, BoxedError> {
        Ok(byte)
    }

    #[inline]
    fn xtime_2(&self, byte: u8) -> Result<u8, BoxedError> {
        let mut res = byte;

        if res & 0x80 == 0x80 {
            res = 0x1b ^ (res << 1);
        } else {
            res = res << 1;
        }

        Ok(res)
    }

    #[inline]
    fn xtime_3(&self, byte: u8) -> Result<u8, BoxedError> {
        // let l = self.xtime_1(word.clone())?;
        // let r = self.xtime_2(word.clone())?;

        // let res = self.addition(l, r)?;

        let res = self.addition(self.xtime_2(byte)?, self.xtime_1(byte)?)?;

        Ok(res)
    }

    fn addition(&self, l: u8, r: u8) -> Result<u8, BoxedError> {
        Ok(l ^ r)
    }
}
